...
for eof_dim in eof_d:
    ...
    for region in reg:
        ...
        for year in yr:
            ...
            print()
            print('Processing:  region -> ' + region + ',  eof component -> ' + str(eof_dim+1) + ',  years -> ' + str(year) + '-' + str(year+time_step))
            print()

            selection = cdo.selyear(str(year) + '/' + str(year+time_step), input = region_prec)
                
            # Extracting data and operating transormation
            prec_fields, latitude, longitude, time = extract_nc(selection, 'tp', True, 'hours since 1950-01-01', 'proleptic_gregorian')
            ...
            plt.savefig('Grafici/Eof/Eof_wind/period_eof' + str(eof_dim+1) + '_' + region + str(year) + '_' + str(year+time_step) + '.png')